/*Problem statement
https://community.topcoder.com/stat?c=problem_statement&pm=8193
*/
...
public class BeautifulString {
	...
	public int maximumLength(int countA, int countB, int maxA, int maxB) {
		...
		for (int i = 0; i < maxBlocks.length; i++) {
			maxBlocks[i] = getBlock(counts[i], maxs[i]);
		}
		int maxLength = 0;
		for (int i = 0; i < 2; i++) {
			int diffs[] = { -1, 0, 1 };
	...
	int getBlock(int count, int max) {
	...
	public static void main(String[] args) {
		...
		System.out.println(new BeautifulString().maximumLength(countA, countB, maxA, maxB));
